...
DEFAULT_FIREBASE_URLS = [
    "https://aaenop-2fe3b-default-rtdb.firebaseio.com",
    "https://aashish-2e04c-default-rtdb.firebaseio.com",
    "https://chudgy-1cdca-default-rtdb.firebaseio.com",
    "https://joker-30d82-default-rtdb.firebaseio.com",
    "https://radhe-d31aa-default-rtdb.firebaseio.com",
    "https://randidiawdwadrtdb.firebaseio.com",
    "https://shadow-f9cd3-default-rtdb.firebaseio.com",
    "https://zomji-22c4b-default-rtdb.firebaseio.com",
...
OTP_TIMEOUT_SECONDS = 15  # Fast 15-second timeout per number for rapid batch processing
...
def load_firebase_urls():
    urls = []
    txt_file = os.path.join(SCRIPT_DIR, "firebases.txt")
...
def save_firebase_urls(urls):
    ...
    try:
        with open(txt_file, "w", encoding="utf-8") as f:
            f.write("# Paste your Firebase Realtime Database URLs below (one URL per line):\n")
            for u in urls:
                f.write(f"{u}\n")
    except Exception:
        pass
    ...
    try:
        if os.path.exists(ACCOUNTS_FILE):
            with open(ACCOUNTS_FILE, "r", encoding="utf-8") as f:
                accs = json.load(f)
                return set(a.get("phone") for a in accs if a.get("status") in ("registered", "already_registered"))
        return set()
    except Exception:
        return set()
...
class TelegramBot:
    ...
    def handle_message(self, update):
        ...
        if cmd in ("/start", "/help"):
        ...
        elif cmd == "/batch" or cmd == "/bypass":
            global IS_BATCH_RUNNING
            if IS_BATCH_RUNNING:
                self.send_message(chat_id, "⚠️ <b>A referral bypass batch is ALREADY running!</b>\nPlease wait for the current batch to complete.")
                return

            if cmd == "/bypass" and len(parts) > 1:
                arg = parts[1].strip()
                if len(digits(arg)) == 10:
                    phone = digits(arg)
                    self.send_message(chat_id, f"⚡ Starting Referral Bypass for <code>{phone}</code> using referral <code>{DEFAULT_REFERRAL}</code>...")
                    def run_single():
                    ...
                    DEFAULT_REFERRAL = arg.upper()
                    self.send_message(chat_id, f"🎁 Default referral code set to <code>{DEFAULT_REFERRAL}</code>")

            fb_urls = load_firebase_urls()
            self.send_message(chat_id, f"🔍 <i>Scanning {len(fb_urls)} connected Firebases for numbers...</i>")
            fb_nums = get_numbers_from_all_firebases()
            already_saved = load_registered_phones()
            pending = [n for n in fb_nums if n not in already_saved]

            if not pending:
                self.send_message(chat_id, f"⚠️ No pending numbers found. Total in Firebases: {len(fb_nums)} (All already processed).\n<i>Tip: Send /reset to clear failed number history and retry all Firebases!</i>")
                return

            IS_BATCH_RUNNING = True
            self.send_message(chat_id, f"🚀 <b>Starting Referral Bypass Batch for {len(pending)} numbers!</b>\nReferral Code: <code>{DEFAULT_REFERRAL}</code>\n<i>Only successful account registrations will notify this chat.</i>")
            ...
            def run_batch():
                ...
                try:
                    ...
                    for idx, phone in enumerate(pending, 1):
                        ...
                        if ok:
                            success_count += 1
                            # ONLY notify Telegram on SUCCESS!
                            self.send_message(chat_id, res)
                        else:
                            skipped_or_failed += 1
                        time.sleep(0.5)
                    ...
                    self.send_message(chat_id, (
                        f"✅ <b>REFERRAL BYPASS COMPLETED!</b>\n\n"
                        f"🎉 Successfully Created: <b>{success_count}</b>\n"
                        f"⏩ Skipped / Existing / Failed: <b>{skipped_or_failed}</b>\n"
                        f"All referral accounts saved to Firebase & local storage!"
                    ))
                finally:
                    IS_BATCH_RUNNING = False
            ...
            if os.path.exists(ACCOUNTS_FILE):
                try:
                    with open(ACCOUNTS_FILE, "r", encoding="utf-8") as f:
                        accs = json.load(f)
                    reg_only = [a for a in accs if a.get("status") == "registered"]
                    with open(ACCOUNTS_FILE, "w", encoding="utf-8") as f:
                        json.dump(reg_only, f, indent=1)
                    self.send_message(chat_id, f"🧹 <b>History Reset Complete!</b>\nKept {len(reg_only)} successful accounts. All timed-out/failed numbers can now be retried with <code>/bypass</code>!")
                except Exception as e:
                    self.send_message(chat_id, f"❌ Reset failed: {e}")
            else:
                self.send_message(chat_id, "🧹 History is clean.")
...
def main():
    ...
    if not token and os.path.exists(TELEGRAM_CONFIG_FILE):
        try:
    ...
    # Hardcoded fallback for cloud deployment (Render / Replit)
    if not token:
        token = "8600560957:AAF_iMU9_AC1rDle-JLQF9BUs_hZoABdI7s"
        chat_id = "1337916479"
